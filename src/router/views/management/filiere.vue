						...
						<div class="rounded-lg shadow-lg bg-white max-w-sm">
							...
							<div class="p-6">
								...
								<p class="text-gray-700 text-base mb-4">
									{{ item.name }} Some quick example text to build on the card title and make up the bulk of the card's
									content.hy jhjh
								</p>
								<h6>{{ item.manager }}</h6>
...
export default {
	...
	data() {
		const filiereSchema = {
			img(value) {
				...
				return "Vous devez choisir une image de profile pour la filiere";
			},
			name(value) { return isLength(value, { min: 6, max: 20 }) ? true : "Le minimum de caracteres est 6 et le maximum 12" },
			manager(value) { return isLength(value, { min: 6, max: 20 }) ? true : "Le minimum de caracteres est 6 et le maximum 12" },
			sub_manager(value) { return isLength(value, { min: 6, max: 20 }) ? true : "Le minimum de caracteres est 6 et le maximum 12" },
			description(value) { return isLength(value, { min: 6, max: 500 }) ? true : "Le minimum de caracteres est 6 et le maximum 500" },
			long_name(value) { return isLength(value, { min: 6, max: 12 }) ? true : "Le minimum de caracteres est 6 et le maximum 12" },
		}
	...
	methods: {
		...
		clickOutside() { },
		onInvalidFiliere({ values, result, errors }) { console.log(errors); }
	},
};
</script>
