<template>
  <div>
    ...
    <div class="card mt-4 min-h-[200px] relative">
      ...
      <ol class="border-l md:border-l-0 md:border-t border-gray-300 md:flex md:justify-center md:gap-6 mt-2" :class="{ 'border-none': exper_state_modif, 'justify-start': userData.experiences.length == 1 }">
        <li v-for="({ position, start, end, company, id }, index) in experiences" :key="index" class="relative" :class="{ 'border-2 border-dashed rounded-lg pl-5': exper_state_modif }">
          <div class="flex md:block flex-start items-center pt-2 md:pt-0">
            <div class="bg-green-300 w-2 h-2 rounded-full -ml-1 md:ml-0 mr-3 md:mr-0 md:-mt-1"></div>
          ...
          <button v-if="educ_state_modif" @click="deleteExperience($route.params.id, id)" class="absolute inline-block bottom-0 right-0 text-center items-center row bg-red-100 rounded-tl-md rounded-br-sm" data-mdb-ripple="true" data-mdb-ripple-color="danger">
            <box-icon type="regular" name="trash" color="red" size="sm" class="text-green-900"></box-icon>
          </button>
        </li>
      </ol>
...
<script setup>
  ...
  onMounted(async () => {
    userData.value = store.employees.find((employee) => employee.id == route.params.id)
  ...
  watch(
    () => route.params.id,
    async (newId) => {
      userData.value = store.employees.find((employee) => employee.id == newId)
      console.log(store.employees)
      console.log(userData.value)
    }
  )
  ...
  async function deleteExperience(idEmployee, experienceID = "4") {
    const result = await store.deleteExperience(idEmployee, experienceID)
    if (result) {
      toast.success(`Remove Education with id ${educationID}`)
    }
  }
