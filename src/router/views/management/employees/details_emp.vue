  ...
  <div>
    ...
    <transition name="fadeSlideX" mode="out-in">
      <div class="" v-if="currentTab == 'basic information'">
          ...
          <div class="row relative transition-all duration-700 w-full">
            <div class="flex-none relative h-[150px] w-[150px] mr-3 items-center align-middle">
              <div class="row items-center my-auto h-full">
                <div class="backdrop-blur-sm bg-red-white/30 absolute left-[25%] top-[35%] z-10 w-20 h-7 rounded-md text-white text-center cursor-pointer" v-if="edit_mode">
                  <button type="button" @click="changepicture">Edit</button>
                </div>
                <img
                  :src="`http://localhost:3000/resources/file/${userData.profile_image}`"
        ...
        <div class="col card mt-4 min-h-[200px] relative transition-all ease-in duration-700 justify-between">
          ...
          >
            <li
              v-for="({ name, start, description, end, from_school, id }, index) in userData.educations"
              :key="index"
    ...
    <MyModal v-show="showModalAddEducation" @close="showModalAddEducation = false">
      ...
      <Form
        ...
        @submit="addEducation"
        v-slot="{ isSubmitting }"
        :validation-schema="educationSchema"
        :initial-values="educationValue"
    ...
    <MyModal v-show="showModalAddExper" @close="showModalAddExper = false">
      ...
      <Form
        ...
        v-slot="{ isSubmitting }"
        :validation-schema="experienceSchema"
        :initial-values="experienceValue"
        @invalid-submit="onInvalidEducation"
      >
        <div class="flex sm:flex-col md:flex-row md:justify-between">
          <div class="w-full">
